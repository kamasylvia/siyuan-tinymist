...
### 软链开发(2026-07-23 链路通,待思源 UI 启用插件)
- [x] 跑 `pnpm install`
- [x] 跑 `pnpm run make-link` → `workspace/data/plugins/siyuan-tinymist` 软链到 `dev/`
- [x] 跑 `pnpm run dev` 填充 `dev/`(index.js/i18n/plugin.json 等)
- [ ] 思源"设置 → 集市 → 开发者模式"启用 siyuan-tinymist(`loadPetals` 当前返 `[]`)
- [ ] `pnpm run dev` watch 模式改代码热重载验证

> **tinymist 二进制**:本机装在 `/Volumes/Ext SSD/bin/tinymist`(v0.15.2,GitHub Releases 预编译 darwin-arm64,已 `xattr -d com.apple.quarantine`)。未入 PATH,需在插件设置 `tinymistPath` 填该绝对路径,或 `ln -s` 到 `/usr/local/bin`。
...
- [x] preview tab + WebSocket SVG 渲染 —— iframe 嵌 tinymist 自带前端页(reflexo WASM renderer),后者连 tinymist data-plane WebSocket 收 vector IR + 增量渲染成 SVG/Canvas + 双向同步。**技术调研定论(2026-07-23)**:tinymist v0.13+ 的 preview 协议已从旧 typst-preview 的「简单 SVG delta」升级为「vector IR + WASM 渲染」,前端 reflexo 是该协议的唯一参考实现;自写「WebSocket 接 SVG delta 自渲染」= 重做 reflexo WASM 管线,工作量巨大且无收益(reflexo 已是最佳实现)。故 **iframe 方案定为正式选型**,非临时占位。实测 preview 页含 `reflexo`/`svg`/`canvas` 渲染产物,HTTP 200
...
> **自动化验证(2026-07-23 已完成,commit 3dbc46b / c9aee3b)**

- [x] `pnpm run build` 无错
- [x] tinymist v0.15.2 二进制可用(`/Volumes/Ext SSD/bin/tinymist`,PATH 外)
- [x] preview stdout 解析正则命中:`Static/Data plane server listening on: 127.0.0.1:<port>`,manager.ts 正则 `\s*(\S+)` 正确抓取
- [x] preview 前端页可达:`GET http://127.0.0.1:<port>` → 200 + reflexo WASM HTML(node http.get 实测)
- [x] spawn 参数修正:`--partial-rendering` 需 `=true`(v0.13+);缺 `--no-open` 会弹浏览器,已补
- [x] kernel API 连通:`/api/export/exportMdContent`(返 `{content,hPath}`)、`/api/attr/getBlockAttrs`(返 IAL 含 custom-*)、`/api/attr/setBlockAttrs`(写/读/null 清除全通)
- [x] **端到端物化链路**(用真实思源文档):API 建 ` ```typst ` 测试文档(docId `20260723103118-5hxhxjx`)→ exportMdContent 取 markdown → block-to-typ 提取 typst 块 → tinymist preview spawn → 正则抓 `127.0.0.1:58180` → HTTP 200 → preview 页含 `reflexo`/`svg`/`canvas`(reflexo WASM 渲染产物),编译错 0
- [x] 端口占用识别:实测绑占用端口 → tinymist Rust panic `Address already in use`,manager.ts `ADDR_IN_USE_RE` 命中(已实现 `TinymistPortInUseError`)
- [x] 软链就位:`workspace/data/plugins/siyuan-tinymist` → `dev/`(经 `pnpm run dev` 填充)
- [x] `git status` 干净(仅 TODO.md 跟踪,无 demo 数据)

> **待本机 GUI QA(需用户在思源桌面交互,纯代码无法完成)**

> ⚠️ **QA 前置**:确保 `dev/` 是最新代码(软链指向 dev/)。改代码后须 `pnpm run dev`(watch)刷新 dev/,否则思源加载的是过期 index.js。dev watch 进程可常驻后台,改代码自动重编译热重载。2026-07-23 已重新 build dev/(166KB,含 fetcher/anchor/settings/错误处理全部新代码)。

- [ ] 思源"设置 → 集市 → 开发者模式"启用 siyuan-tinymist(loadPetals 当前返 `[]`,需 UI 启用)
- [ ] tinymistPath 留默认 `tinymist`(fetcher 自动下载到 `<pluginData>/bin/`),或填 `/Volumes/Ext SSD/bin/tinymist` 跳过下载
- [ ] topbar 眼睛图标 / `⇧⌘P` 触发 preview,preview tab 显示 tinymist 渲染页
- [ ] tinymist 进程随插件卸载/思源退出正确清理(`onunload` kill,不留僵尸 —— 可 `pgrep -fl "tinymist preview"` 验证)
- [ ] 典型 Typst 文档(`$E=mc^2$` 数学公式、`#import` 多文件、中文)渲染正确 —— 测试文档已建 `tinymist-qa-test`(docId `20260723103118-5hxhxjx`)
- [ ] 入口锚点 4 层场景:IAL `custom-typst-root` / 会话 pin / 物化默认 各手动验证
...
- [x] **tinymist 二进制分发**:**决策=首次运行下载**(对齐 VSCode tinymist 扩展),已落地 `src/tinymist/fetcher.ts`。`ensureTinymistBinary` 流程:用户 settings 路径 > PATH 查找 > `<pluginData>/bin/` 缓存 > 按 platform/arch 从 GitHub Release v0.15.2 下载 + SHA256 校验 + 解压(容忍 `tinymist-<triple>/tinymist` 顶层目录)+ chmod + 去 macOS quarantine。实测完整链路通过(下载 30MB / sha match / 解压 / --version)。index.ts openPreview 前 ensureBinary,下载进度 showMessage
- [ ] **block → typ 映射策略**:起步用策略 1(已实现)。策略 3(混合 wasm 内联预览)待定 —— tinymist 架构下内联预览需在 renderer 跑 reflexo WASM(同 iframe 方案),ROI 低,暂不做
- [x] **preview 容器形态**:定为 **iframe 嵌 tinymist 自带前端页(reflexo WASM)**。技术依据:tinymist v0.13+ preview 用 vector IR + WASM 渲染(非旧 SVG delta),reflexo 是唯一参考实现,自渲染=重做 WASM 管线无收益。iframe 方案完整复用渲染+双向同步,样式可控性通过 tinymist `--invert-colors` / preview-args(sys.inputs)部分调节。详见 §4 交付清单第 2 项
- [x] **思源版本兼容**:`minAppVersion` 暂留 `3.2.1`(模板值)。所用 API(exportMdContent/getBlockAttrs/setBlockAttrs/addTab/getAllEditor/fetchSyncPost)均为思源稳定 API,3.x 系列通用;精确下限待多版本实测(GUI QA 后)
- [x] **CI / GitHub Actions**:`.github/workflows/release.yml` 已就位(tag `v*` → build → 上传 package.zip 到 Release)。功能 A 代码已完成,可启用 —— 打 tag 即触发
- [ ] **市集上架**:待 GUI QA 验证通过后,PR [`siyuan-note/plugin-gallery`](https://github.com/siyuan-note/plugin-gallery) 提交元数据。需遵守市集规范(plugin.json 字段已齐)
