...
- [ ] **tinymist 二进制分发**:**决策=首次运行下载**(对齐 VSCode tinymist 扩展)。首包小,按平台/架构从 GitHub Release 下载对应预编译二进制到 `<pluginData>/bin/`,校验 SHA256。否决 bundle(包体 20-30MB × 多平台)、否决纯预装(普通用户门槛高)。PATH 有 tinymist 则跳过下载。落地代码 `src/tinymist/fetcher.ts`(待实现,§4 交付后)
