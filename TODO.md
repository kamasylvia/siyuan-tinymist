...
- [ ] tinymist 进程管理(spawn + 生命周期 + 配置)
- [ ] preview tab + WebSocket SVG 渲染
- [ ] block → `main.typ` 物化(策略 1)
- [ ] 入口锚点 4 层解析 + UI
- [ ] 设置页(tinymist 路径、端口、渲染模式)
- [ ] 错误处理:tinymist 不存在/端口占用/编译失败的友好提示
- [ ] 仅桌面端守卫(`isMobile()`/`hasNodeEnv()`,移动端禁用并提示)
- [ ] 本地化(中英)
- [ ] README + 使用文档
...
2. ✅ **模板清理(§3)** → commit `ffb4eae`(push 待用户确认)
3. ⏳ 软链开发链路通(`make-link` + 思源加载,需本机 QA)
4. ⏳ 功能 A 起步:tinymist 进程管理 + 最小 preview tab(策略 1,单文件)
